...
- [ ] **시간 동기화 표시** 여러 PC의 이벤트를 합칠 때 시계 편차가 그대로 오차가 된다. 하트비트에 PC 시각을 넣어 수집 PC 시각과 비교, 편차가 크면 타일에 경고. (현황판 Grid.Tile.Editor `/live` 는 하트비트 `time` 으로 편차를 재어 `시계 +3분` 배지를 붙임 — 2026-09-16. 수집 모드 타일은 아직.)
...
- [ ] **입력 경로 결정 대기 (장치 확정 후 진행)** 2026-09-04 결론: 어떤 산업용 리더가 들어올지 정해지지 않아 아래 둘은 보류.
  ...
  - **MQTT 구독 프로바이더(슬림)** — 장치가 MQTT 를 직접 내거나 게이트웨이(Kepware, Node-RED, Ignition 등)를 거칠 때. 설정 모델(`MqttReaderSettings`), 비밀번호 DPAPI 보호(`Mqtt/Secret.cs`), JSON 경로 해석기(`Mqtt/MqttPayloadParser.cs`)는 이미 들어가 있고, **MQTTnet 5.2 참조는 발행 싱크(`Output/MqttSink.cs`, 2026-09-14)와 함께 들어왔음**. 남은 것: `MqttReaderProvider`(브로커별 접속 공유, 재접속, 토픽 매칭, StaleMs), 등록 폼, 리더 도구 탭 표시. 토픽 탐색기·내장 브로커는 만들지 않음(MQTT Explorer, MQTTX 로 대체). 견적: Modbus 의 절반.
  - 일반 OPC UA(규격 없는 벤더 노드)는 만들지 않음. 미들웨어로 MQTT 변환 후 위 MQTT 경로 사용.
- [x] **MQTT 발행 싱크** 리더별 현재 상태(retained) · 등장/제거 이벤트(QoS 1, 로컬 큐) · PC 상태(retained + LWT). Grid Tile Editor `/live` 현황판이 구독. 설정 탭 "MQTT 브로커로 발행". 페이로드 형식 **v1** 은 `docs/mqtt/` 의 JSON Schema 로 고정하고 `tests/RfidReaderMonitor.Tests` 가 실제 발행 코드의 출력을 검증(CI `dotnet test`). 모든 JSON 출력에 `v` 필드가 실린다. (2026-09-14) v0.4.1: 뽑힌 순간 S/N 을 못 읽어 이름 키로 나가던 유령 retained 를 막음 — 이름별 마지막 S/N 을 기억해 같은 키를 유지하고, S/N 을 알게 되면 이름 · 별명 키 토픽을 빈 retained 로 지운다. (2026-09-15)
- [x] **MQTT 재발행 요청** 현황판(이력 기록기)이 죽어 있던 구간을 `{prefix}/{site}/replay` · `…/host/{PC}/replay` 로 요청하면 CSV 출력(`events-yyyyMMdd.csv`)에서 그 구간을 읽어 `…/reader/{key}/replay` 로 다시 내고 `…/host/{PC}/replay-done` 으로 건수를 알림(`Output/MqttReplay.cs`, 스키마 `docs/mqtt/replay*.schema.json`). 이벤트 토픽과 분리해 현황판의 지금 상태는 흔들리지 않음. Grid Tile Editor `/live` 의 `빠진 이력 요청` 폼이 요청자. 2026-09-17 저녁 Grid 기록기가 15시간 죽어 이력이 사라진 뒤 넣음. (v0.5.0, 2026-09-18)
